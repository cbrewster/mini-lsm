...
impl SsTableIterator {
    ...
    pub fn seek_to_key(&mut self, key: KeySlice) -> Result<()> {
        ...
        // Key was not found in block, set the iterator to the
        // start of the next block, if one exists.
...
impl StorageIterator for SsTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        ...
        if !self.blk_iter.is_valid() && self.blk_idx + 1 < self.table.block_meta.len() {
            ...
            self.blk_iter =
                BlockIterator::create_and_seek_to_first(self.table.read_block_cached(idx)?);
            self.blk_idx = idx;
        }
